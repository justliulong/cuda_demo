...
// 主机函数
__host__ void hello() {  // tip 也可以不使用 __host__ ，因为默认就是主机函数
    std::cout << "hello host \n";
}
...
std::pair<int, int> getConvOutSize(int input_h, int input_w, int kernel_size,
                                   int stride = 1, int padding = 0, int dilation = 0) {
...
int main() {  //! 这里没有修饰符，实际上就是主机函数，这里面可以调用其他的主机函数，也能调用核函数，但是不能调用设备函数
    ...
    // Use CUDA events for precise GPU-side timing (avoids CPU overhead & page fault interference)
    ...
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);

    std::cout << std::format("Init array time: {} microseconds.\n", static_cast<int>(elapsed_ms * 1000.0f));
    ...
    */
    ...
    int* res;
    CUDA_CHECK(cudaMallocManaged(&res, gridSize * sizeof(int)));
    cudaMemset(res, 0, gridSize * sizeof(int));
    int* s;
    CUDA_CHECK(cudaMallocManaged(&s, sizeof(int)));
    cudaMemset(s, 0, sizeof(int));

    // Warm-up: run all sum kernels once to eliminate GPU cold-start overhead
    ...
    // sum1 使用 memset 查看page fault 带来的性能影响
    ...
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    std::cout << std::format("sum1 array time (with memset) by using global memory: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), *s);

    // sum1: global memory atomic add
    cudaMemset(s, 0, sizeof(int));
    cudaEventRecord(c_start);
    sum1<<<gridSize, blockSize>>>(arr2, s, n);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    std::cout << std::format("sum1 array time (with cudaMemeset) by using global memory: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), *s);

    // sum2: shared memory, forward reduce
    cudaMemset(res, 0, gridSize * sizeof(int));
    cudaEventRecord(c_start);
    sum2<<<gridSize, blockSize>>>(arr2, res, n);
    cudaEventRecord(c_stop);
    ...
    for (int i = 0; i < gridSize; ++i)
        sum += res[i];
    std::cout << std::format("sum2 array time by using share memory: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), sum);

    // sum3: shared memory, reverse reduce (less warp divergence)
    cudaMemset(res, 0, gridSize * sizeof(int));
    cudaEventRecord(c_start);
    sum3<<<gridSize, blockSize>>>(arr2, res, n);
    cudaEventRecord(c_stop);
    ...
    for (int i = 0; i < gridSize; ++i)
        sum += res[i];
    std::cout << std::format("sum3 array time by using optimized share memory: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), sum);

    // sum4: shared memory + warp reduce (template)
    cudaMemset(res, 0, gridSize * sizeof(int));
    cudaEventRecord(c_start);
    sum4<blockSize><<<gridSize, blockSize, blockSize * sizeof(int)>>>(arr2, res, n);
    cudaEventRecord(c_stop);
    ...
    for (int i = 0; i < gridSize; ++i)
        sum += res[i];
    std::cout << std::format("sum4 array time by using share memory and warp: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), sum);

    // sum5: warp shuffle (register-only reduce)
    cudaMemset(res, 0, gridSize * sizeof(int));
    cudaEventRecord(c_start);
    sum5<<<gridSize, blockSize>>>(arr2, res, n);
    cudaEventRecord(c_stop);
    ...
    for (int i = 0; i < gridSize; ++i)
        sum += res[i];
    std::cout << std::format("sum5 array time by using warp shuffle: {} microseconds.  res is {} \n",
                             static_cast<int>(elapsed_ms * 1000.0f), sum);
    ...
    cudaEventRecord(c_start);
    mul1<<<blocks, threads>>>(d_A, d_B, d_C, M, N, K);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    ...
    cudaEventRecord(c_start);
    mul2<<<blocks, threads>>>(d_A, d_B, d_C, M, N, K);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    std::cout << std::format("mul array time by using share memory: {} microseconds. \n",
                             static_cast<int>(elapsed_ms * 1000.0f));
    ...
    dim3 blocks_3D((K + threads_3D.x - 1) / threads_3D.x, (M + threads_3D.y - 1) / threads_3D.y, (B + threads_3D.z - 1) / threads_3D.z);  // 还是要注意这里使用的 K(列) 和 M(行) 的顺序

    cudaEventRecord(c_start);
    mul_3d1<<<blocks_3D, threads_3D>>>(d_3D_A, d_3D_B, d_3D_C, B, M, N, K);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    ...
    blocks_3D = {(K + threads_3D.x - 1) / threads_3D.x, (M + threads_3D.y - 1) / threads_3D.y, (B + threads_3D.z - 1) / threads_3D.z};

    cudaEventRecord(c_start);
    mul_3d2<<<blocks_3D, threads_3D>>>(d_3D_A, d_3D_B, d_3D_C, B, M, N, K);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    cudaEventElapsedTime(&elapsed_ms, c_start, c_stop);
    ...
    std::cout << std::format("=========================== computer conv ========================================\n");
    // 比如我的卷积核大小为 3 * 3， 里面的值均为 1
    const int kernel_size = 3;
    const int half_kernel = kernel_size / 2;
    float* h_kernel = new float[kernel_size * kernel_size];
    std::fill(h_kernel, h_kernel + kernel_size * kernel_size, 1);

    // 将 核的数据 拷贝到常量内存的 kernel_data 中
    cudaMemcpyToSymbol(kernel_data, h_kernel, kernel_size * kernel_size * sizeof(float), 0, cudaMemcpyHostToDevice);  //

    // kernel
    float* d_k;
    cudaMalloc(&d_k, kernel_size * kernel_size * sizeof(float));
    cudaMemcpy(d_k, h_kernel, kernel_size * kernel_size * sizeof(float), cudaMemcpyHostToDevice);

    // 创建初始的输入数据
    const int input_size = 4096;
    float* h_input = new float[input_size * input_size];
    std::fill(h_input, h_input + input_size * input_size, 1);
    ...
    // 创建输出数据
    auto [out_h, out_w] = getConvOutSize(input_size, input_size, kernel_size);
    float* h_output = new float[out_h * out_w];
    std::fill(h_output, h_output + out_h * out_w, 0);

    // 拷贝数据
    float* d_output;
    cudaMalloc(&d_output, out_h * out_w * sizeof(float));
    cudaMemcpy(d_output, h_output, out_h * out_w * sizeof(float), cudaMemcpyHostToDevice);

    // 线程块
    dim3 threadsPerBlock(16, 16);
    // 网格
    dim3 blockPregrid((out_h + threadsPerBlock.x - 1) / threadsPerBlock.x, (out_w + threadsPerBlock.y - 1) / threadsPerBlock.y);
    // conv1<<<blockPregrid, threadsPerBlock>>>(d_input, d_k, d_output, input_size, input_size, kernel_size, out_w, out_h);
    // conv2<<<blockPregrid, threadsPerBlock>>>(d_input, d_output, input_size, input_size, kernel_size, out_w, out_h);
    const int STRIDE = 1;
    const int SHARE_H = threadsPerBlock.y * STRIDE + 2 * half_kernel;
    const int SHARE_W = threadsPerBlock.x * STRIDE + 2 * half_kernel;
    // Warm-up call, then timed call
    conv3<<<blockPregrid, threadsPerBlock, SHARE_H * SHARE_W * sizeof(float)>>>(d_input, d_output, input_size, input_size, kernel_size, out_w, out_h, STRIDE);
    cudaEventRecord(c_start);
    conv3<<<blockPregrid, threadsPerBlock, SHARE_H * SHARE_W * sizeof(float)>>>(d_input, d_output, input_size, input_size, kernel_size, out_w, out_h, STRIDE);
    cudaEventRecord(c_stop);
    cudaEventSynchronize(c_stop);
    ...
    std::cout << std::format("conv take time : {:.2f} ms\n", elapsed_ms);

    // 将结果拷贝回主机内存
    CUDA_CHECK(cudaMemcpy(h_output, d_output, out_h * out_w * sizeof(float), cudaMemcpyDeviceToHost););
